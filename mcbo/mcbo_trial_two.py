...
def mcbo_trial(
    ...
    for i in range(len(X)):
        intervention_set_identifiers = list(map(lambda x: int(x), X[i].numpy()[0:6].tolist()))
        if 1 in intervention_set_identifiers:
            intervention_set = []
            intervention_levels = []
            for j, num in enumerate(intervention_set_identifiers):
                if num == 1:
                    intervention_set.append(cbo_config.scm.variables[j])
                    intervention_level = network_observation_at_X[i][j]
                    intervention_levels.append(intervention_level.item())
            if intervention_set != ["C", "D"]:
                continue
            intervention_level = np.array(intervention_levels)
            ...
            if len(cbo_config.scm.interventions[cbo_config.scm.exploration_set.index(intervention_set)][
    ...
    while results.remaining_budget > 0:
        ...
        intervention_set_identifiers = list(map(lambda x: int(x), new_x.numpy()[0][0:6].tolist()))
        if 1 in intervention_set_identifiers:
            intervention_set = []
            intervention_levels = []
            for j, num in enumerate(intervention_set_identifiers):
                if num == 1:
                    intervention_set.append(cbo_config.scm.variables[j])
                    intervention_level = new_x.numpy()[0][6:].tolist()[j]
